...
const databaseName = `meditation_knowledge_e2e_${randomBytes(6).toString('hex')}`;
...
try {
  ...
  run('pnpm', ['exec', 'vitest', 'run', 'apps/api/src/e2e/knowledge.e2e.test.ts'], {
    env: { ...process.env, DATABASE_URL: databaseUrl, RUN_KNOWLEDGE_E2E: 'true' },
  });
} finally {
  try {
