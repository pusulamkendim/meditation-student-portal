...
export class FieldEncryption {
  ...
  encrypt(plaintext: string, associatedData?: string): EncryptedValue {
    return this.encryptBuffer(Buffer.from(plaintext, 'utf8'), associatedData);
  }

  encryptBuffer(plaintext: Buffer, associatedData?: string): EncryptedValue {
  ...
  decrypt(value: EncryptedValue, associatedData?: string): string {
    return this.decryptBuffer(value, associatedData).toString('utf8');
  }

  decryptBuffer(value: EncryptedValue, associatedData?: string): Buffer {
